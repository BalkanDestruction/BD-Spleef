...
import fr.naruse.spleef.v1_13.api.event.cancellable.reason.SpleefCommandPerformedEvent;
...
public class SpleefCommands implements CommandExecutor, TabExecutor {
    ...
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String ss, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            if (args[0].equalsIgnoreCase("block")) {
                if (args.length < 2) {
                    return sendMessage(sender, "§3Hey! §6/§cspleef block <Add, Remove, List> <[Type]>");
                }
                ...
                if (args[1].equalsIgnoreCase("remove")) {
                    ...
                    if (pl.getConfig().getStringList("v1_13.blocks").contains(args[2].toUpperCase())) {
                        List<String> list = pl.getConfig().getStringList("v1_13.blocks");
                        list.remove(args[2].toUpperCase());
                        pl.getConfig().set("v1_13.blocks", list);
                        pl.saveConfig();
    ...
    private boolean help(CommandSender sender, int page) {
        if (page == 1) {
            ...
            sendMessage(sender, "§3Hey! §6/§cspleef block <Add, Remove, List> <[Type]> <[Data]>");
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length == 3) {
            if (args[0].equalsIgnoreCase("block") && args[1].equalsIgnoreCase("add")) {
                for (Material material : Material.values()) {
                    if (material.toString().contains(args[2].toUpperCase())) {
